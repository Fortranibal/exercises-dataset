...
import { CHART, kde, monthColor } from "@/lib/charts/theme";
...
function buildRidges(
  ...
  return months.map((month) => {
    ...
    return {
      ...
      density: xs.map((x) => ({ x, y: kde(vals, x, bandwidth) })),
    };
  });
}
...
function insightLine(ridges: Ridge[], proteinFloor: number): string {
  if (ridges.length < 2) {
    return "Smoothed daily-intake densities per month.";
  }
  const late = ridges.slice(-2);
  const drifted = late.some((r) => {
    const peak = r.density.reduce((a, b) => (b.y > a.y ? b : a), r.density[0]);
    return peak.x < proteinFloor;
  });
  if (drifted) {
    const names = late.map((r) => r.label).join("/");
    return `Smoothed daily-intake densities per month · protein ridges drift below the ${proteinFloor}g floor in ${names}.`;
  }
  return "Smoothed daily-intake densities per month · peaks relative to protein floor and calorie target.";
}
  ...
  return (
    <section className="chart-panel overflow-hidden">
      ...
      <div className="flex justify-end border-t border-white/6 px-5 py-3 md:px-7">
        <p className="text-[11px] text-[var(--muted)]">{rangeLabel}</p>
      </div>
    </section>
  );
}
